...
class AdyenPayByLinkController(AdyenController):
    ...
    def adyen_notification(self, **post):
        """ Process the data sent by Adyen to the webhook based on the
        event code.

        See https://docs.adyen.com/development-resources/
        webhooks/understand-notifications for the
    ...
    def _verify_notification_signature(notification_data, tx_sudo):
        ...
        :param dict notification_data: The notification payload containing the
                                       received signature
        :param recordset tx_sudo: The sudoed transaction referenced by
                                  the notification data, as a
                                  `payment.transaction` record
        :return: None
        :raise: :class:`werkzeug.exceptions.Forbidden`
                        if the signatures don't match
        """
        # Retrieve the received signature from the payload
        received_signature = notification_data.get(
            'additionalData.hmacSignature')
        if not received_signature:
            _logger.warning("received notification with missing signature")
            raise Forbidden()

        # Compare the received signature with the expected
        # signature computed from the payload
    ...
    def _compute_signature(payload, hmac_key):
        ...
        :param str hmac_key: The HMAC key of the acquirer handling the
                             transaction
        ...
        def _flatten_dict(_value, _path_base='', _separator='.'):
            ...
            :param Object _value: The value to flatten. A dict or an already
                                  flat value
            :param str _path_base: They base path for keys of _value, including
                                   preceding separators
            :param str _separator: The string to use as a separator in the key
                                   path
            """
            # The inner value is a dict, flatten it
            if isinstance(_value, dict):
                _path_base = (_path_base if not _path_base else
                              _path_base + _separator)
                for _key in _value:
                    yield from _flatten_dict(
                        _value[_key], _path_base + str(_key))
            else:  # The inner value cannot be flattened, yield it
                yield _path_base, _value

        def _to_escaped_string(_value):
            """ Escape payload values that are using illegal symbols and cast
            them to string.
        ...
        # Calculate the HMAC with the binary representation of the signing
        # string with SHA-256
        binary_hmac = hmac.new(binary_hmac_key, signing_string.encode('utf-8'),
                               hashlib.sha256)
