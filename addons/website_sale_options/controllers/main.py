...
from openerp.addons.website_sale.controllers.product import ProductController


class WebsiteSaleOptions(ProductController):

    @http.route(['/shop/product/<model("product.template"):product>'],
                type='http', auth="public", website=True)
    def product(self, product, category='', search='', **kwargs):
        _super = super(WebsiteSaleOptions, self)
        r = _super.product(product,
                           category,
                           search, **kwargs)
        cr, uid, context, pool = \
        ...
        for p in product.optional_product_ids:
            ...
            optional_product_ids.append(template_obj.browse(cr, uid, p.id,
                                        context=ctx))

        r.qcontext['optional_product_ids'] = optional_product_ids
        return r

    @http.route(['/shop/cart/update_option'],
                type='http', auth="public", methods=['POST'], website=True)
    def cart_options_update_json(self, product_id, add_qty=1,
                                 set_qty=0, goto_shop=None, **kw):
        cr, uid, context, pool = \
            request.cr, request.uid, request.context, request.registry

        order = request.website.sale_get_order(force_create=1)
        product = pool['product.product'].browse(cr, uid, int(product_id),
                                                 context=context)

        option_ids = [p.id
                      for tmpl in product.optional_product_ids
                      for p in tmpl.product_variant_ids]
        optional_product_ids = []
        for k, v in kw.items():
            if "optional-product-" in k and \
                    int(kw.get(k.replace("product", "add"))) and \
                    int(v) in option_ids:
